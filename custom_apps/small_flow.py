...
def quick_advection(field, velocity_field, dt, type_id="default"):
    try:
        ...
        print("Velocity Grid (v, u) at (i,j):\n ", vel, "\n")
        # Position: (i+0.5,j)
        points2 = CenteredGrid( np.array(list(map(lambda row: np.array(list(map(lambda pos: pos + [0.0, 0.5], row))), points.data))) )
        ...
        print("Sample Points (i+0.5,j):\n ", points2.data, "\n")
        print("u at (i+0.5,j):\n ", vel2_u, "\n")
        # Position: (i+0.5,j+1)
        points3 = CenteredGrid( np.array(list(map(lambda row: np.array(list(map(lambda pos: pos + [1.0, 0.5], row))), points.data))) )
        vel3 = velocity_field.sample_at(points3.data)[0]
        vel3_u = np.array(list(map(lambda row: np.array(list(map(lambda pos: [pos[1]], row))), vel3)))
        print("Sample Points (i+0.5,j+1):\n ", points3.data, "\n")
        print("u at (i+0.5,j+1):\n ", vel3_u, "\n")
        # u at Position: (i+0.5,j+0.5) # Equation (3)
        vel4_u = 0.5 * (vel_u + vel2_u)
        print("u at (i+0.5,j+0.5):\n", vel4_u, "\n")


        raise NotImplementedError("QUICK Advection is WIP!")
...
def quick_advection_old(field, velocity_field, dt, type_id="default"):
    try:
        points = field.points
        # Field values
        values = field.sample_at(points.data)[0]
