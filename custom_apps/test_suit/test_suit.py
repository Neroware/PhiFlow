    ...
    def save_gradients(self):
        if not semi_langrange_mode:
            ...
            with tf.Session("") as sess:
                ...
                plot_grid(grd_field.eval()[0], self.name, self.name + "_den_grad.jpg", -0.004, 0.004)
                plot_grid(grd_u.eval()[0], self.name, self.name + "_u_grad.jpg", -0.004, 0.004)
                plot_grid(grd_v.eval()[0], self.name, self.name + "_v_grad.jpg", -0.004, 0.004)
                sess.close()
